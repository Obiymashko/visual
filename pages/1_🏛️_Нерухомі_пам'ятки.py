...
# Палітри кольорів для кожного блоку
...
with st.container(border=True):
    ...
    with top_right0:
        ...
        values0 = [65891, 35654, 11892, 2567, 198, 177, 121]
        fig0 = go.Figure(
            data=[
                go.Pie(
                    labels=labels0,
...
with st.container(border=True):
    ...
    with top_right4:
        labels4 = [
            ...
            "З геоданими",
            "Підготовлено",
            "Не оцифровано (залишок)",
        ]
        values4 = [178, 15, 158, 405 - (178 + 15 + 158)]
        fig4 = go.Figure(
            data=[
                go.Pie(
                    labels=labels4,
...
with st.container(border=True):
    ...
    with top_right5:
        labels5 = [
            "Верифіковано Спадщиною",
            "Без зауважень УДПД",
            "Очікують верифікації",
        ]
        values5 = [65, 0, 1043 - 65]
        fig5 = go.Figure(
            data=[
                go.Pie(
                    labels=labels5,
                    values=values5,
                    hole=0.6,
                    marker_colors=colors5 + ["#e5e7eb"],
                    textinfo="none",
                )
            ]
        )
        fig5 = clean_chart_layout(fig5, height=150)
...
def load_data(path, sheet):
    ...
    df = df[(df["Регіон"].str.lower() != "nan") & (df["Регі violent"].str.len() > 3)] if "Регі violent" in df.columns else df[(df["Регіон"].str.lower() != "nan") & (df["Регіон"].str.len() > 3)]
